...
import { MdLibraryBooks, MdSearch, MdAdd, MdMoreVert } from 'react-icons/md'
    ...
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Manage all academic courses</p>
        </div>
      ...
      {error ? (
        ...
        <div className="flex flex-col items-center justify-center min-h-[200px] p-8 text-center bg-white/70 backdrop-blur-xl rounded-2xl border border-white/30 shadow-md">
          <div className="w-14 h-14 rounded-xl bg-gray-100 flex items-center justify-center mb-3">
            <MdLibraryBooks size={28} className="text-gray-400" />
          </div>
          <p className="text-sm text-gray-500">{search ? 'No courses match your search' : 'No courses found'}</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((course, i) => (
            ...
            >
              ...
              <h3 className="text-base font-semibold text-gray-800 mb-1">{course.name}</h3>
              <p className="text-xs font-mono text-gray-400 uppercase mb-3">{course.code}</p>
