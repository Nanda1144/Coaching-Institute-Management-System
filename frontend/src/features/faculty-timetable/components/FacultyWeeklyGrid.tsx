...
interface FacultyWeeklyGridProps {
  entries: FacultyScheduleEntry[]
}

const dayOrder = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday']
const timeSlots = ['09:00 AM - 10:00 AM', '10:00 AM - 11:00 AM', '11:00 AM - 12:00 PM', '02:00 PM - 03:00 PM', '03:00 PM - 04:00 PM']

export default function FacultyWeeklyGrid({ entries }: FacultyWeeklyGridProps) {
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50/50">
              ...
              {dayOrder.map((day) => (
                <th key={day} className="text-center px-3 py-3 font-semibold text-gray-600 text-xs uppercase tracking-wider min-w-[150px]">
                  {day.slice(0, 3)}
                </th>
              ))}
            </tr>
                    ...
                    <td key={day} className="px-2 py-2 text-center">
                      {entry ? (
                        <div className={`rounded-lg border p-2 ${
                          entry.status === 'Ongoing' ? 'border-emerald-300 bg-emerald-50/50' :
                          entry.status === 'Completed' ? 'border-gray-200 bg-gray-50/30' :
                          'border-blue-100 bg-blue-50/30'
                        }`}>
                          <p className="font-semibold text-gray-800 text-[11px] leading-tight">{entry.subject}</p>
                          <p className="text-[10px] text-gray-500 mt-0.5">{entry.batch}</p>
